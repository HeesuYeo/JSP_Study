...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (map != null && map.containsKey(id)) {
			...
			if (id.equals("")) {
				out.print("<script>alert('ID를 입력하세요.'); location.href='/chap04/quiz/index.jsp'</script>");
				out.close();
				return;
			} else if (pw.equals("")) {
				out.print("<script>alert('PW를 입력하세요.'); location.href='/chap04/quiz/index.jsp'</script>");
				out.close();
				return;
			}
						
			out.print("<script>alert('존재하지 않는 ID입니다.'); location.href='/chap04/quiz/index.jsp'</script>");
