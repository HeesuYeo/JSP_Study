...
public class EncodingServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException { 
		...
		/*
		  	GET 방식 파라미터는 URL 뒤에 데이터가 추가되어 도착한다.
		  	URL의 해석은 WAS(Apache-Tomcat)가 담당하기 때문에
		  	해당 값을 올바른 인코딩 타입으로 해석하기 위해서는 서버의 설정을 변경해야 한다.
		  	(server.xml에서 포트번호 수정한 곳(63번째 줄)에 URIEncoding="EUC-KR"을 추가)
		 */
		
		// samkim이랑 smoothie 받아서 출력하기
		System.out.println(req.getParameter("samkim"));
		System.out.println(req.getParameter("smoothie"));
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		*/
		...
		// (다른 JSP 파일로 포워드를 하면 기본적으로 셋팅이 되어있기 때문에 이 과정 생략 가능)
