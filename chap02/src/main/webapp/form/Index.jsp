...
<body>
	...
	<ul>
		...
		<li>form 태그 내부의 input 태그들을 통해 사용자가 선택한 값을 서버로 보낼 수 있다.</li>
		<li>데이터를 key-value 방식으로 실어보낸다.</li>
	...
	<!-- 
	 ...
	 <ul>
	 	...
	 	<li>주소창에 URL을 직접 입력하거나, 링크를 클릭하는 방식은 기본적으로 GET 방식이다.</li>
	 </ul>
	...
	<form action="http://localhost:9000/chap02/test1" method="GET">
		...
		커피: <input type="password" name="coffee" value="americano"> <br>
	...
	<ul>
		<li>URL에 직접 입력하거나 링크를 통해 전달할 수 없다. (form 또는 javascript를 이용해야 함)</li>
		<li>
			주소 뒤에 함께 실려가는 방식이 아니라 눈에는 보이지 않는 형태로 전달된다.
			(패킷을 열어보면 모든 정보를 볼 수는 있다.)
		</li>
		<li>request 객체의 body라는 필드에 실려서 전송된다.</li>
		<li>비밀번호 같은 것들은 GET 방식으로 보내면 다 보이기 때문에 POST 방식을 이용해야 한다.</li>
	</ul>
	
	<!-- 
		웹 상에서 상대경로를 쓸 때는 현재 URL이 어디인지를 파악해야 한다.
	 ...
	 <!-- form 바깥에 있는 input도 form 속성을 설정하면 소속으로 만들 수 있다. -->
